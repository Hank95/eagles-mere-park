...
- **2026-05-11** — Auth code exchange happens in a Route Handler at `src/app/auth/callback/route.ts`, never on a Server Component page. Server Components cannot reliably write the Supabase auth cookies that `exchangeCodeForSession` needs to persist — our `src/lib/supabase/server.ts` `setAll` callback intentionally swallows the cookie-write error in that context. Recovery emails, invite acceptance, and any future OAuth flow (Google sign-in, email change confirmation) all redirect through `/auth/callback?code=...&next=<safeNext target>`. The Route Handler exchanges the code (cookie writes succeed because Route Handlers can set cookies), then redirects to the validated `next` path. Considered and rejected during Phase 1: doing the exchange inside the `/update-password` Server Component page — silent cookie-write failure produces "Auth session missing" on subsequent submission.
- **2026-05-12** — Member-row claim runs from the authed layout, not from /auth/callback. Helper `claimMemberRowIfNeeded` (in `src/lib/members/claim.ts`) runs after `auth.getUser()` on every signed-in request; fast-paths out when already linked, otherwise SELECTs an unclaimed row by lowercased email match and UPDATEs `user_id`. Idempotent per-request. Rationale: handles post-invitation email changes; cost is one indexed SELECT that short-circuits on the common case; the alternative (claim only on first sign-in via `/auth/callback`) couldn't handle the rare email-changed-after-invite case. Two narrow RLS policies (`members_claim` UPDATE in migration `20260512122422` and `members_self_select_for_claim` SELECT in migration `20260512221015`) widen Phase 0's policies just enough to allow the self-claim while still preventing any other claim path.
- **2026-05-12** — Inline edit on the same URL via `?edit=1`, not a separate `/households/<id>/edit` route. The same page renders read view or edit form depending on the query param and `canEdit` (admin OR same-household). Rationale: one URL, one mental model; admin edit-any is a one-line conditional rather than two route trees. Unauthorized `?edit=1` access redirects back to the read view.
- **2026-05-12** — Per-field privacy on contact fields; address visibility uses an any-member-public rule. Each member's `email_is_public`, `phone_is_public`, `address_is_public` flags are toggled in the edit form. Read view filters per `(public_flag || isAdmin || sameHousehold || isSelf)`. Address lives on `households.street_address` but the privacy gate is per-member: `shouldShowAddress` returns true for admin/same-household, otherwise true iff any member has `address_is_public` set. Rationale: warm default (a household with even one opted-in member is reachable for contact); the alternative (require majority or all) feels paternalistic.
- **2026-05-13** — RSVPs are per-household with optional headcount, not per-member (deviation from §5.2's earlier per-member sketch). Schema: `rsvps(event_id, household_id, status, headcount)` with unique `(event_id, household_id)`. Rationale: most community events are 50+ attendees; "Pendleton family is in, bringing 4" is the natural unit; granular per-member RSVPs would add clicks for every multi-person household and the "shows count + list of attendees" UX reads cleanly with household-level aggregation. Per-member granularity can be revisited if a specific event type demands it.
- **2026-05-13** — Display timezone is hardcoded to America/New_York; storage is UTC. Helpers in `src/lib/events/format.ts` round-trip `<input type="datetime-local">` strings via `easternDateInputValue` / `easternDateFromInput` (the latter probes both EDT and EST offsets so DST works without a date library). `isDatetimeLocalString` guards `easternDateFromInput` against malformed input from crafted POSTs. `easternIso` projects a UTC instant to its Eastern calendar date for grouping and day-filter logic. Rationale: single-tenant, single-location project; per-user TZ preference would be overhead for no benefit.
- **2026-05-13** — Calendar is a month-grid view only; no week or agenda toggle. The chronological feed at `/events` IS the agenda view. A week view would duplicate the feed and complicate URL state for no real gain at our scale.
- **2026-05-13** — RLS policies may use cross-table EXISTS subqueries when (and only when) the gating data cannot be encoded as a JWT claim. The canonical example: `rsvps_insert`/`rsvps_update` policies subquery `events.rsvp_enabled` so a direct API POST cannot create an RSVP on an event with RSVPs disabled. AGENTS.md's "confirm a JWT claim can't do the job first" requirement is satisfied because `events.rsvp_enabled` is per-row and changeable; encoding it in the JWT would require an auth re-issue on every event edit. Reuse this pattern for future row-level gates on related tables (e.g., a future "gallery is locked" flag gating photo uploads).
- **2026-05-13** — Placeholder-then-illustration deferral pattern for asset-dependent features. Phase 4 built the entire map infrastructure (cottages table, SVG host, popovers, pan/zoom, admin authoring) against a placeholder SVG of labeled rectangles, with `/map` gated to admins via `isAdmin(user)` until the real illustration arrives. When the asset ships, the swap is small: replace the SVG component + the placeholder data file, rerun `pnpm seed-cottages`, remove the admin gate, add the nav link. Rationale: avoids serializing on the external illustrator dependency; proves the interaction layer in isolation; honors PLANNING.md §6's "a mediocre map damages identity more than a missing map" warning by hiding the placeholder from members. Reusable pattern for any feature waiting on external assets (e.g., a hero illustration for the public landing page, a finished policy document).
- **2026-05-13** — Pan/zoom uses `panzoom` (anvaka), not `react-svg-pan-zoom` or hand-rolled. ~10kb gzipped, framework-agnostic, handles mouse drag + touch + pinch + bounds + momentum. Rejected `react-svg-pan-zoom` (heavier, less idiomatic for our usage) and hand-rolling (50-80 LOC of pointer-event handling for no real benefit). Known quirk: clicks may fire after a drag — manageable with drag-threshold heuristics if needed in the future. Initialized in a `useEffect` on the panzoom-target div; cleaned up in the effect's return.
- **2026-05-13** — Mobile bottom sheets use a custom ~30-line slide-up `<div>`, not `vaul`/shadcn `Drawer`. The behaviors needed (backdrop click-to-close, drag-handle visual, body-scroll lock, `aria-modal="true"`) are constrained enough that ~15kb of additional dependency isn't justified. Pattern: render with `fixed inset-0 z-20 flex flex-col justify-end`, full-screen `<button>` backdrop, sheet container with `max-h-[60vh] overflow-y-auto rounded-t-xl`, body `overflow: hidden` while open. If a future sheet needs nested drags, swipe-to-dismiss, or stacking, revisit and consider `vaul`.
- **2026-05-13** — Server Actions that UPDATE by id must pass `{ count: "exact" }` and return a friendly error when 0 rows matched. Supabase `update().eq("id", x)` does not error on 0-row updates — both "id doesn't exist" and "RLS silently blocked the write" return `{ error: null, data: null }`. Without a `count` check, the action returns `{ saved: true }` despite no DB change, misleading the caller. Pattern: `const { error, count } = await supabase.from(t).update(p, { count: "exact" }).eq("id", id); if (error) return { error: ... }; if (count === 0) return { error: "<Resource> not found." };`. Apply to any future `update<Resource>` action; consider retrofitting `updateHousehold` and `updateEvent` in a future polish pass.
